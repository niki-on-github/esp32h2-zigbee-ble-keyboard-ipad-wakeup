...
{
    if (num_key > HID_KEYBOARD_IN_RPT_LEN ) {
        ESP_LOGE(HID_LE_PRF_TAG, "%s(), the number key should not be more than %d", __func__, HID_KEYBOARD_IN_RPT_LEN - 2);
        return;
    }
    ...
    ESP_LOGI(HID_LE_PRF_TAG, "the key value = %02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x", buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7]);
    hid_dev_send_report(hidd_le_env.gatt_if, conn_id,

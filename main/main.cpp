...
static const char *TAG = "BLE_HID_WAKEUP";

#define HID_DEMO_TAG "HID_WAKEUP"
#define HIDD_DEVICE_NAME "DashboardWakeup"
...
void setLED(bool value) {
        if (sec_conn) {
            ESP_LOGI(HID_DEMO_TAG, "Send Spacebar Press");
            uint8_t key_value[16] = {0};
            key_value[0] = HID_KEY_SPACEBAR;
            esp_hidd_send_keyboard_value(hid_conn_id, 0, key_value, 1);
            vTaskDelay(50 / portTICK_PERIOD_MS); 
            ESP_LOGI(HID_DEMO_TAG, "Send Spacebar Release");
            uint8_t release_key[5] = {0};
...
extern "C" void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Bluetooth initialized successfully");
      ...
      zbLight.setManufacturerAndModel("Espressif", "ZBDashboardWakeup");
      zbLight.onLightChange(setLED);
      Zigbee.addEndpoint(&zbLight);

      if (!Zigbee.begin()) {
        ESP_LOGE(HID_DEMO_TAG, "%s Zigbee failed to start!", __func__);
        ESP.restart();
      }
    ESP_LOGI(TAG, "Connecting to Zigbee network");
        ...
        if (digitalRead(button) == LOW) {  // Push button pressed
                ...
                while (digitalRead(button) == LOW) {
                  ...
                  if ((millis() - startTime) > 3000) {
                    ...
                    ESP_LOGE(HID_DEMO_TAG, "%s Resetting Zigbee to factory and rebooting in 1s.", __func__);
                    delay(1000);
                    Zigbee.factoryReset();
                  }
                }
